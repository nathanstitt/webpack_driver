...
class TestConfiguration < MiniTest::Test
    ...
    def test_webpack_config_adds_plugin
        config = WebpackDriver::Configuration.new(
            '/tmp/test-webpack.config.js', port: 233
        )
        refute config.present?
        assert_equal(config.port, 233)
        assert_includes(
            Pathname.new(config.path).read,
            "var config = require('/tmp/test-webpack.config.js')"
        )
    end

    def test_flags
        config = WebpackDriver::Configuration.new(
            '/tmp/test-webpack.config.js', port: 233, environment: 'dev'
        )
    ...
    def test_configuration_generation
        Dir.mktmpdir do | dir |
            config = WebpackDriver::Configuration.new("#{dir}/webpack.config.js")
            refute config.present?
            package = Class.new {
